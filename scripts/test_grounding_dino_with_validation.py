...
def draw_boxes_with_scores(image, boxes, target_names, scores, kept_indices=None):
    ...
    for idx, (box, name) in enumerate(zip(boxes, target_names)):
        ...
        total_h = sum((sz[1] for sz in line_sizes), 0) + line_gap * (len(lines) - 1 if lines else 0)
        ...
        cv2.rectangle(
            ...
            (min(result.shape[1] - 1, text_x + max_w + 2 * pad), min(result.shape[0] - 1, y_start + total_h + pad)),
            color,
            -1,
        )
        ...
        for (t, (w_i, h_i)) in zip(lines, line_sizes):
            cv2.putText(result, t, (text_x + pad, y_cursor), font, font_scale, (255, 255, 255), thickness)
...
def main():
    parser = argparse.ArgumentParser(description="Test GroundingDINO + sample validation")
    parser.add_argument(
        "--camera",
        action="store_true",
    ...
    logger.info(f"Loaded {len(matcher.samples)} reference samples: {list(matcher.samples.keys())}")
...
def process_frame(image, gd_predictor, matcher, args, frame_num=None):
    ...
    if not boxes:
        logger.warning(f"Frame {frame_num}: No boxes detected" if frame_num else "No boxes detected")
...
def run_camera_mode(gd_predictor, matcher, args):
    ...
    logger.info("Camera initialized. Press 'q' to quit, 's' to save frame, 'p' to pause")
    logger.info(f"Targets: {args.targets}")
