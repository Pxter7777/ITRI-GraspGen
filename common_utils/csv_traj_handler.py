...
def run_trajectory(command: str, obstacles: list | None = None, vel=40, acc=20, blend=100):
    ...
    for node in nodes:
        ...
        if node.mode == Mode.OPEN:
            movements.append(SingleRobotMove(type="gripper", grip_type="open", wait_time=1.5))
        elif node.mode == Mode.CLOSE:
            movements.append(SingleRobotMove(type="gripper", grip_type="close", wait_time=0.9))
        elif node.mode == Mode.HALF_OPEN:
            movements.append(SingleRobotMove(type="gripper", grip_type="half_open", wait_time=0.5))
        elif node.mode == Mode.CLOSE_TIGHT:
            movements.append(SingleRobotMove(type="gripper", grip_type="close_tight", wait_time=0.9))
        elif node.mode == Mode.MOVE:
            if move_at_least_once:
                movements.append(SingleRobotMove(type="sequence_joint_rad", sequence_joint_rad_goals=node.joints_values, vel=vel, acc=acc, blend=blend, no_curobo=True))
            else:
                move_at_least_once = True
                movements.append(SingleRobotMove(type="sequence_joint_rad", sequence_joint_rad_goals=node.joints_values, vel=vel, acc=acc, blend=blend))
