...
from src import config, mouse_handler, sam_utils, realsense_utils, stereo_utils, visualization, pointcloud_utils
...
def main():
    ...
    parser.add_argument('--ckpt_dir', default=f'{code_dir}/FoundationStereo/pretrained_models/23-51-11/model_best_bp2.pth',
    ...
    # ---------- RealSense Init ----------
    pipeline, align, K_ir1, K_color, ext_ir1_to_color, baseline, (W, H) = realsense_utils.initialize_realsense()
    ...
    try:
        while True:
            ...
            if mouse_handler.drawing_box:
                visualization.draw_box(display_frame, mouse_handler.box_start_point, mouse_handler.box_end_point)
            ...
            disp, (H_scaled, W_scaled) = stereo_utils.run_stereo_inference(stereo_model, ir1_np, ir2_np, stereo_args)
            
            vis_disp = visualization.vis_disparity(disp)
            vis_disp_resized = cv2.resize(vis_disp, fx=1/stereo_args.scale, fy=1/stereo_args.scale, dsize=None)
            ...
            if key == 32 and mouse_handler.box_defined:
                pointcloud_utils.save_scene_and_obj(
                    stereo_args, K_ir1, baseline, disp, ext_ir1_to_color, 
                    K_color, color_np_org, mask, combined_vis
                )
